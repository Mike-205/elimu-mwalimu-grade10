...
- **Learning outcomes are verbatim; the teaching content around them is not.** Every `learningOutcomes` entry is quoted from the official KICD design and machine-checked against the extracted source in `corpus/_source/` by `verify-outcomes.js`. The board notes, activity, questions and explainer are written for this project, because the design specifies outcomes and not pedagogy. Each pack states this split rather than implying the whole thing is official. The earlier plan to OCR a scanned copy is obsolete — text-layer copies of both designs were found and `pdftotext` extracts them directly.
- **Coverage is complete for both of Jane's subjects and nothing else.** 31 sub-strands across 6 strands: Computer Science (17) and Mathematics (14). Any other Grade 10 subject still returns `sijui`, which is the correct behaviour rather than a bug, but it does mean the tool is useful to a teacher only if their timetable happens to match Jane's. Adding a subject is a matter of adding JSON, not code.
- **One sub-strand in the Mathematics design cannot be offered.** Its contents page names "Angle Properties of a Circle" but the body of the document contains no such section, so there are no published outcomes to quote. See `docs/corpus-content-plan.md`.
- **The `verified` badge colour is not driven by the data.** `public/app.js` reads the badge *text* from `meta.verified` but hardcodes the CSS class, so every pack renders in the illustrative styling regardless of value. This errs conservative and misrepresents nothing, but the colour is currently decorative and should be derived from `verified` now that more than one value exists.
